...
if __name__ == "__main__":
    ...
    # print(e2lpd)
    df = pd.DataFrame.from_dict(e2lpd, orient='index')


    df = df.rename(columns={0: "prediction"})
    df.index.rename('question', inplace=True)
    ...
    if datatype == 'categorical':
        ...
        print(full_df)
        acc = metrics.accuracy_score(full_df['truth'], full_df['prediction'])
        if len(full_df['truth'].unique()) > 2:
            print("weighted")
            f1 = metrics.f1_score(full_df['truth'], full_df['prediction'], average='weighted')
        else:
